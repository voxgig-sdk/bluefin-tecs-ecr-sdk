# BluefinTecsEcr Haskell SDK



The Haskell SDK for the BluefinTecsEcr API — an entity-oriented client following idiomatic Haskell conventions (pure functions, explicit `IO`, and the dependency-free vendored `Value` struct model).

The SDK exposes the API as capitalised, semantic **Entities** — for example `ecr_api sdk VNoval` — each
...
release tag (`haskell/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/bluefin-tecs-ecr-sdk/releases)) or
...
```haskell
  createEnt <- Sdk.ecr_api sdk VNoval
...
| `ecr_api` | `Client -> Value -> IO Entity` | Create an EcrApi entity instance. |
...
Create an instance: `ecr_api <- Sdk.ecr_api sdk VNoval`
...
└── bluefintecsecr-sdk.cabal      -- package manifest (for Hackage)
```
