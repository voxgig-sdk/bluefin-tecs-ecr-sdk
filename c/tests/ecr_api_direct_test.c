// Generated direct-call test for the ecr_api entity (mirrors the
...
static voxgig_value* ecr_api_mock(void* ud, voxgig_value* args) {
  CALLS++;
...
static BluefinTecsEcrSDK* ecr_api_direct_setup(voxgig_value* mockres) {
  voxgig_value* opts = cmap(2,
    "base", v_str("http://localhost:8080"),
    "system", cmap(1, "fetch", vfn(ecr_api_mock, mockres)));
  return bluefin_tecs_ecr_sdk_new(opts);
}
...
int main(void) {
  ...
  // LOAD
  {
    ...
    BluefinTecsEcrSDK* sdk = ecr_api_direct_setup(mockres);
  ...
  TEST_SUMMARY("bluefin_tecs_ecr_ecr_api_direct");
}
