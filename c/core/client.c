// BluefinTecsEcrSDK client (generated — mirrors the rust Main fragment).
...
BluefinTecsEcrSDK* bluefintecsecr_sdk_new(voxgig_value* options) {
  BluefinTecsEcrSDK* sdk = (BluefinTecsEcrSDK*)calloc(1, sizeof(BluefinTecsEcrSDK));
...
static bool sdk_op_allowed(BluefinTecsEcrSDK* sdk, const char* op) {
...
static voxgig_value* sdk_op_denied(BluefinTecsEcrSDK* sdk, const char* op) {
  voxgig_value* allow_op = getpath2(sdk->options, "allow", "op");
...
static voxgig_value* sdk_raw_request(
  BluefinTecsEcrSDK* sdk, voxgig_value* fetchargs, PNError** err) {
  *err = NULL;
  Utility* utility = sdk->utility;
...
voxgig_value* sdk_graphql(BluefinTecsEcrSDK* sdk, const char* query,
  ...
  *err = NULL;
  ...
  if (voxgig_is_list(errors) && 0 < voxgig_as_list(errors)->len) {
    ...
    char msg[512];
    snprintf(msg, sizeof(msg), "BluefinTecsEcrSDK: graphql: %s", m);
...
BluefinTecsEcrSDK* test_sdk(voxgig_value* testopts, voxgig_value* sdkopts) {
  ...
  BluefinTecsEcrSDK* sdk = bluefintecsecr_sdk_new(sdkopts);
  free(sdk->mode);
  sdk->mode = strdup("test");
  return sdk;
}
