...
class EcrApiDirectTest extends TestCase
{
    public function test_direct_load_ecr_api(): void
    {
        $setup = ecr_api_direct_setup(["id" => "direct01"]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-load-ecr_api", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
...
function ecr_api_direct_setup($mockres)
{
    ...
    if ($live) {
        ...
        $client = new BluefinTecsEcrSDK($merged_opts);
        return [
            "client" => $client,
            "calls" => $calls,
            "live" => true,
