...
class EcrApiEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new EcrApiEntity($this->_client, $opts);
    }

    /**
     * @param EcrApi|array $args EcrApi data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = BluefinTecsEcrHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return EcrApi|array The current EcrApi data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of EcrApi fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = BluefinTecsEcrHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of EcrApi fields).
     */
     ...
     *
     * @param EcrApiLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed EcrApiLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return EcrApi|array The loaded EcrApi as an assoc-array at the
     *   SDK boundary; throws BluefinTecsEcrError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = BluefinTecsEcrHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
     ...
     * Create a new EcrApi.
     *
     * @param EcrApiCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed EcrApiCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return EcrApi|array The created EcrApi as an assoc-array at the
     *   SDK boundary; throws BluefinTecsEcrError on failure (item-5 convention).
     */
