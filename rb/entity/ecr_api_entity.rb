...
class EcrApiEntity
  ...
  def make
    opts = @_entopts.dup
    EcrApiEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = BluefinTecsEcrHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [EcrApi, Hash] the current EcrApi data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = BluefinTecsEcrHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of EcrApi fields)
  def match_get
  ...
  #
  # @param reqmatch [EcrApiLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.EcrApi.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [EcrApi, Hash] the loaded EcrApi; raises BluefinTecsEcrError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = BluefinTecsEcrHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  # Create a new EcrApi.
  #
  # @param reqdata [EcrApiCreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [EcrApi, Hash] the created EcrApi; raises BluefinTecsEcrError on failure
