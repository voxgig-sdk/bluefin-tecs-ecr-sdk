// Generated direct-call tests for the ecr_api entity (unit mode;
...
static EcrApiDirectSetup ecr_api_direct_setup(const Value& mockres) {
  Value calls = vlist();
  Value cshared = calls;
  ...
  auto client = std::make_shared<BluefinTecsEcrSDK>(opts);
...
static void ecr_api_direct_load() {
  auto setup = ecr_api_direct_setup(vmap({{"id", Value("direct01")}}));
  auto sk = is_control_skipped("direct", "direct-load-ecr_api", "unit");
...
int main() {
  T_RUN(ecr_api_direct_load);
  return sdktest::summary("ecr_api_direct_test");
}
