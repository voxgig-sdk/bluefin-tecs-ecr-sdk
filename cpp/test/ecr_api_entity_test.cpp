...
static EcrApiSetup ecr_api_basic_setup(const Value& extra) {
  ...
  Value env = env_override(vmap({
    ...
    {"BLUEFIN_TECS_ECR_TEST_LIVE", Value("FALSE")},
    {"BLUEFIN_TECS_ECR_TEST_EXPLAIN", Value("FALSE")}
  }));

  Value idmap_resolved = Helpers::toMapAny(getp(env, "BLUEFIN_TECS_ECR_TEST_ECR_API_ENTID"));
  if (!idmap_resolved.is_map()) idmap_resolved = idmap;

  bool live = getp(env, "BLUEFIN_TECS_ECR_TEST_LIVE") == Value("TRUE");
...
static void ecr_api_entity_basic() {
  ...
  {
    Value ecr_api_ref01_data_result = ecr_api_ref01_ent->create(Struct::clone(ecr_api_ref01_data), Value::undef())->data();
  ...
  Value ecr_api_ref01_data_dt0_loaded = ecr_api_ref01_ent->load(ecr_api_ref01_match_dt0, Value::undef())->data();
