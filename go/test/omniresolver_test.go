...
import (
	...
	omni "github.com/voxgig-sdk/bluefin-tecs-ecr-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.BluefinTecsEcrError); is {
				out := map[string]any{
					"name":    "BluefinTecsEcrError",
...
func omniCtx(arg any, client *sdk.BluefinTecsEcrSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.BluefinTecsEcrSDK); is {
			client = live
