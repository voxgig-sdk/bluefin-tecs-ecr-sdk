# Vendored struct corpus runner for the BluefinTecsEcr SDK.
#
...
defmodule BluefinTecsEcr.StructCorpus do
  ...
  defp pclient_for(primary, name) do
    ...
    if S.ismap(setup), do: BluefinTecsEcr.test(nil, S.clone(setup)), else: BluefinTecsEcr.test()
  end
  ...
  def run_primary(testfile \\ nil) do
    ...
    client = BluefinTecsEcr.test()
    Process.put(:pclient, client)
    Process.put(:pbaseclient, client)
    rootctx = BluefinTecsEcr.get_root_ctx(client)
    ...
    run_primary_set("makeContext", psec(primary, "makeContext"), fn e ->
      ...
      BluefinTecsEcr.Context.new(ctxmap, nil)
    end)
    ...
    run_primary_set("operator", psec(primary, "operator"), fn e ->
      inv = eget(e, "in")
      op = BluefinTecsEcr.Operation.new(if(S.ismap(inv), do: inv, else: S.jm([])))
      S.jm([
