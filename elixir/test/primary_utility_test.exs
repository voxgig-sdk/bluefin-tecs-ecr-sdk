...
defmodule BluefinTecsEcr.PrimaryUtilityTest do
  use ExUnit.Case

  alias Voxgig.Struct, as: S
  alias BluefinTecsEcr.Omni, as: O
  alias BluefinTecsEcr.Utility, as: U
  ...
  defp livectx(node, opts) do
    ...
    util = BluefinTecsEcr.get_utility(sdk)
    rootctx = BluefinTecsEcr.get_root_ctx(sdk)

    ctxmap = S.clone(node)
    S.delprop(ctxmap, "client")
    S.delprop(ctxmap, "utility")

    ctx = BluefinTecsEcr.Context.new(ctxmap, nil)
    ...
    if S.ismap(specmap), do: S.setprop(ctx, "spec", BluefinTecsEcr.Spec.new(specmap))

    resmap = S.getprop(ctxmap, "result")

    if S.ismap(resmap) do
      result = BluefinTecsEcr.Result.new(resmap)
      ...
      if S.ismap(errmap) do
        ...
        if is_binary(msg) and msg != "",
          do: S.setprop(result, "err", BluefinTecsEcr.Error.new("", msg, nil))
      end
    ...
    if S.ismap(respmap) do
      response = BluefinTecsEcr.Response.new(respmap)
  ...
  test "the primary corpus runs through the vendored omni engine", %{run: run, sdk: sdk} do
    ...
    runsection.("makeContext", fn args ->
      ...
      out = BluefinTecsEcr.Context.new(ctxmap, nil)
    ...
    specsdk = BluefinTecsEcr.test(nil, setup_opts(spec, "makeSpec"))
    ...
    runsection.("operator", fn args ->
      op = BluefinTecsEcr.Operation.new(argnode(args, 0))
    ...
    authsdk = BluefinTecsEcr.test(nil, setup_opts(spec, "prepareAuth"))
