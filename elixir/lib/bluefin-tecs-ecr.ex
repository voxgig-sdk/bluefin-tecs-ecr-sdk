...
defmodule BluefinTecsEcr do
  alias Voxgig.Struct, as: S
  alias BluefinTecsEcr.Helpers, as: H
  alias BluefinTecsEcr.{Utility, Context, Spec}
  ...
  def new(options \\ nil) do
    ...
    config = BluefinTecsEcr.Config.shared_config()
    ...
    if S.islist(feature_order) and S.size(feature_order) > 0 do
      Enum.each(0..(S.size(feature_order) - 1), fn i ->
        ...
        if fom != nil and S.getprop(fom, "active") == true do
          Utility.feature_add(rootctx, BluefinTecsEcr.Features.make_feature(fname))
        end
      end)
    end
      ...
      _ ->
        ...
        cond do
          ...
          S.ismap(fetched) ->
            ...
            content_length =
              if S.ismap(headers), do: BluefinTecsEcr.Feature.header_get(headers, "content-length"), else: nil
  ...
  def graphql(client, query, variables \\ nil, ctrl \\ nil) do
    if not op_allowed?(client, "graphql") do
      ...
      if count == 0 do
        ...
        S.setprop(res, "err", "BluefinTecsEcrSDK: graphql: " <> msg)
  ...
  @doc "Entity factory for ecr_api."
  def ecr_api(client, entopts \\ nil) do
    BluefinTecsEcr.Entity.EcrApi.new(client, entopts)
  end
