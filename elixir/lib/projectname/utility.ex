...
defmodule BluefinTecsEcr.Utility do
  alias Voxgig.Struct, as: S
  alias BluefinTecsEcr.Helpers, as: H
  alias BluefinTecsEcr.{Context, Spec, Result, Response, Operation}

  @default_user_agent "Mozilla/5.0 (compatible; BluefinTecsEcrSDK/1.0)"
  ...
  def make_options_impl(ctx) do
    ...
    # (BluefinTecsEcr.Config.shared_config) and merge uses its nested nodes as
  ...
  def make_error_impl(ctx, err) do
    ...
    errmsg =
      cond do
        match?(%BluefinTecsEcr.Error{}, err) -> err.msg
    ...
    msg = "BluefinTecsEcrSDK: " <> opname <> ": " <> (errmsg || "")
    msg = clean(ctx, msg)
    ...
    sdk_err = if match?(%BluefinTecsEcr.Error{}, err), do: %{sdk_err | code: err.code}, else: sdk_err
  ...
  defp err_msg(e) do
    cond do
      match?(%BluefinTecsEcr.Error{}, e) -> e.msg
  ...
  defp safe_json(str) do
    try do
      {:ok, BluefinTecsEcr.Json.parse(str)}
    rescue
      _ -> :error
    end
  end
