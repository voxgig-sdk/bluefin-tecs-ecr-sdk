...
enum OmniResolver {
  ...
  private final class ProviderBox {
    var clients: [ObjectIdentifier: BluefinTecsEcrSDK] = [:]
  ...
  private static func register(_ provider: Omni.Provider, _ client: BluefinTecsEcrSDK,
    _ isdef: Bool)
  {
    providers.lock.lock()
  ...
  private static func defclient(_ provider: Omni.Provider) -> BluefinTecsEcrSDK? {
    providers.lock.lock()
    defer { providers.lock.unlock() }
  ...
  private static func sdkProvider(_ client: BluefinTecsEcrSDK, _ isdef: Bool = false)
    -> Omni.Provider
  {
    ...
    provider.client = { options in
      ...
      return sdkProvider(BluefinTecsEcrSDK.testSDK(nil, opts), true)
    }

    // Client options may reference the runner store.
    provider.inject = { options, store in
      let o = tovalue(options)
      _ = BluefinTecsEcrSdk.inject(o, tovalue(store))
      return tojson(o)
    }
    ...
    provider.errify = { err in
      if let sdkerr = err as? BluefinTecsEcrError {
        var entries: [(String, Omni.Json)] = [
          ("name", .str("BluefinTecsEcrError")),
  ...
  final class Run {
    ...
    let client: BluefinTecsEcrSDK

    private let pack: Omni.RunPack

    fileprivate init(_ pack: Omni.RunPack, _ client: BluefinTecsEcrSDK) {
      self.pack = pack
  ...
  /// itself).
  static func makeRunner(_ testfile: String, _ client: BluefinTecsEcrSDK) throws
    -> NamedRunner
  {
  ...
  static func makeRunner(_ spec: Value, _ client: BluefinTecsEcrSDK) throws
    -> NamedRunner
  {
    return try namedRunner(try Omni.makeRunner(tojson(spec), sdkProvider(client)), client)
  }

  private static func namedRunner(_ runner: Omni.Runner, _ client: BluefinTecsEcrSDK)
    -> NamedRunner
  {
  ...
  static func omniCtx(_ arg: Value, _ client: BluefinTecsEcrSDK, _ utility: Utility)
    -> Context
  {
