...
final class PipelineTest: XCTestCase {
  ...
  private func plClient(_ sdkopts: VMap?) -> (BluefinTecsEcrSDK, Utility) {
    let client = BluefinTecsEcrSDK.testSDK(nil, sdkopts)
    return (client, client.getUtility())
  }

  private func plCtx(_ client: BluefinTecsEcrSDK, _ utility: Utility, _ ctrl: VMap?) -> Context {
  ...
  private func errCode(_ act: () throws -> Void) -> String {
    do { try act(); return "" } catch let e as BluefinTecsEcrError { return e.code } catch { return "" }
  }
  ...
  private func utilWith(_ client: BluefinTecsEcrSDK, _ fetcher: @escaping FetcherFunc) -> Utility {
  ...
  func testMakeRequestTransportErrorCarriedOnResponse() {
    ...
    XCTAssertEqual((resp.err as? BluefinTecsEcrError)?.code, "boom")
  }
