...
import voxgig.bluefintecsecrsdk.core.*
...
object PrimaryCorpus {
  ...
  def toValue(o: Object): Value = o match {
    ...
    case x if x eq voxgig.bluefintecsecrsdk.utility.struct.Struct.UNDEF => VNull
    case m: JMap[_, _] =>
  ...
  private def corpusCtx(client: BluefinTecsEcrSDK, ctxmap: Value): Context = {
  ...
  private def runset(name: String, client: BluefinTecsEcrSDK)(f: (Context, Seq[Value]) => Value): Unit =
  ...
  private def clientFor(name: String, shared: BluefinTecsEcrSDK): BluefinTecsEcrSDK =
    sectionSetup(name) match {
      case m @ VMap(_) => BluefinTecsEcrSDK.testSDK(null, jmap(m))
      case _           => shared
    }
  ...
  def run(testfile: String): Int = {
    ...
    val sdk = BluefinTecsEcrSDK.testSDK()
