...
import voxgig.bluefintecsecrsdk.core._
import voxgig.bluefintecsecrsdk.feature._
...
object SecretsTestMain {
  ...
  private val ENVPREFIX = "BLUEFIN_TECS_ECR_TEST_SECRETS_"
  ...
  private def secretsFeatureOf(client: BluefinTecsEcrSDK): SecretsFeature = {
  ...
  private def withSecrets(build: Boolean => BluefinTecsEcrSDK): BluefinTecsEcrSDK = {
    val client = build(false)
    if (null == secretsFeatureOf(client)) build(true) else client
  }

  private def secretsClient(w: Wire, extra: (String, Object)*): BluefinTecsEcrSDK =
    withSecrets { extend =>
      val opts = sdkopts(w, extra*)
      if (extend) opts.put("extend", jlist(new SecretsFeature()))
      new BluefinTecsEcrSDK(opts)
    }

  private def testModeClient(w: Wire, extra: (String, Object)*): BluefinTecsEcrSDK =
    withSecrets { extend =>
      val opts = sdkopts(w, extra*)
      if (extend) opts.put("extend", jlist(new SecretsFeature()))
      BluefinTecsEcrSDK.testSDK(null, opts)
    }
  ...
  private def entities(client: BluefinTecsEcrSDK): List[SdkEntity] = {
  ...
  private def driveUntil(client: BluefinTecsEcrSDK, stop: () => Boolean): (Boolean, String) = {
    var lasterr = ""
    var done = false
  ...
  private def driveOneCall(client: BluefinTecsEcrSDK, w: Wire): Boolean = {
  ...
  private def testInactive(): Unit = {
    val client = BluefinTecsEcrSDK.testSDK(null, jmap("apikey" -> "OPTKEY01"))
    ...
    val bare = BluefinTecsEcrSDK.testSDK(null, null)
