// Generated direct-call tests for the ecr_api entity (mirrors the java
...
object EcrApiDirectTest {

  private class DirectSetup(val client: BluefinTecsEcrSDK, val calls: JList[JMap[String, Object]])
  ...
  private def directSetup(mockres: Object): DirectSetup = {
    ...
    val client = new BluefinTecsEcrSDK(SdkTestSupport.om(
  ...
  def run(rep: SdkTestReport): Unit = {
    rep.scope("direct-load-ecr_api") {
      ...
      rep.eq("direct-load-ecr_api.ok", java.lang.Boolean.TRUE, result.get("ok"))
      rep.eqI("direct-load-ecr_api.status", 200, Helpers.toInt(result.get("status")))
      rep.check("direct-load-ecr_api.data", result.get("data") != null, "expected data to be non-null")
      val dataMap = Helpers.toMapAny(result.get("data"))
      if (dataMap != null) rep.eq("direct-load-ecr_api.dataId", "direct01", dataMap.get("id"))
      rep.eqI("direct-load-ecr_api.calls", 1, setup.calls.size())
    }
  }
}
