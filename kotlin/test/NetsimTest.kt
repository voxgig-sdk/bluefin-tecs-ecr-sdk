...
class NetsimTest {
  ...
  fun offlineSimulationFailsRequest() {
    val client = BluefinTecsEcrSDK.testSDK(fhMap("net", fhMap("offline", true)), null)
  ...
  fun failstatusSimulationSurfacesStatus() {
    val client = BluefinTecsEcrSDK.testSDK(fhMap("net", fhMap("failTimes", 1, "failStatus", 503)), null)
    val res = client.direct(fhMap("path", "/ping"))
  ...
  fun latencySimulationDelaysRequest() {
    val delay = 60
    val client = BluefinTecsEcrSDK.testSDK(fhMap("net", fhMap("latency", delay)), null)
