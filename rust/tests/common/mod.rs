...
use bluefin_tecs_ecr_sdk::core::helpers::{
    get_bool, get_str, getp, ja, jo, json_thunk, now_ms, setp, to_int, to_map,
};
use bluefin_tecs_ecr_sdk::utility::voxgigstruct as vs;
use bluefin_tecs_ecr_sdk::{
    json_parse, test_sdk, Context, CtxSpec, FeatureRef, FetcherFn, Operation, OutVal,
    BluefinTecsEcrError, BluefinTecsEcrSDK, Response, SdkResult, Spec, Utility, Value,
};
...
/// env_override (mirrors go): when BLUEFIN_TECS_ECR_TEST_LIVE or
/// BLUEFIN_TECS_ECR_TEST_OVERRIDE is TRUE, environment variables replace the
/// defaults in `m` (JSON-shaped values are parsed).
pub fn env_override(m: Value) -> Value {
    let live = std::env::var("BLUEFIN_TECS_ECR_TEST_LIVE").unwrap_or_default() == "TRUE";
    let over = std::env::var("BLUEFIN_TECS_ECR_TEST_OVERRIDE").unwrap_or_default() == "TRUE";
    ...
    if let Ok(explain) = std::env::var("BLUEFIN_TECS_ECR_TEST_EXPLAIN") {
        if !explain.is_empty() {
            setp(&m, "BLUEFIN_TECS_ECR_TEST_EXPLAIN", Value::str(explain));
        }
    }
...
pub struct EntityTestSetup {
    pub client: Rc<BluefinTecsEcrSDK>,
    pub data: Value,
    pub idmap: Value,
    pub env: Value,
    pub explain: bool,
...
pub fn match_string(pattern: &str, val: &str) -> bool {
    if pattern.len() >= 2 && pattern.starts_with('/') && pattern.ends_with('/') {
        return bluefin_tecs_ecr_sdk::utility::voxgigstruct::re::Regex::new(
...
pub fn runset(
    testspec: &Value,
    subject: &mut dyn FnMut(&Value) -> Result<Value, BluefinTecsEcrError>,
) {
    ...
    if let Value::Map(_) = getp(&ctxmap, "result") {
        ...
        if let Value::Map(_) = getp(&res_map, "err") {
            if let Some(msg) = get_str(&getp(&res_map, "err"), "message") {
                result.borrow_mut().err = Some(BluefinTecsEcrError::new("", &msg));
            }
        }
...
pub fn fixctx(ctx: &Rc<Context>, client: &Rc<BluefinTecsEcrSDK>) {
    if ctx.options.borrow().is_noval() {
        *ctx.options.borrow_mut() = client.options_map();
    }
}
...
pub fn err_from_map(m: &Value) -> Option<BluefinTecsEcrError> {
    let msg = get_str(m, "message").unwrap_or_default();
    if msg.is_empty() {
        return None;
    }
    let code = get_str(m, "code").unwrap_or_default();
    Some(BluefinTecsEcrError::new(&code, &msg))
}
...
pub fn fh_has_feature(name: &str) -> bool {
    let config = bluefin_tecs_ecr_sdk::make_config();
...
pub type FhReply = Rc<dyn Fn(i64, &Value) -> Result<Value, BluefinTecsEcrError>>;
...
pub struct FhOpResult {
    pub ok: bool,
    pub data: Value,
    pub err: Option<BluefinTecsEcrError>,
...
fn fh_populate_result(ctx: &Rc<Context>, fetched: &Result<Value, BluefinTecsEcrError>) {
    ...
    *ctx.result.borrow_mut() = Some(result.clone());
    ...
    let resp = Response::new(response_map);
    {
        ...
        if let Value::Func(_) = &resp.json {
            r.body = bluefin_tecs_ecr_sdk::call_json(&resp.json);
        }
...
impl FhHarness {
    ...
    pub fn op(&self, o: FhOpSpec) -> FhOpResult {
        ...
        *ctx.spec.borrow_mut() = Some(spec.clone());
        ...
        let fetched: Result<Value, BluefinTecsEcrError> = match ctx.out_get("request") {
    ...
    fn fail(&self, ctx: Rc<Context>, err: BluefinTecsEcrError) -> FhOpResult {
        {
...
pub fn fh_err_code(err: &Option<BluefinTecsEcrError>) -> String {
