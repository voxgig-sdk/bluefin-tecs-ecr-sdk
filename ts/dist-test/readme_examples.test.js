...
const __1 = require("..");
const SDK_NAME = 'BluefinTecsEcrSDK';
...
const TEST_SEED = { "entity": { "ecr_api": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+BluefinTecsEcrSDK\b/.test(code) ||
        /\bBluefinTecsEcrSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+BluefinTecsEcrSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/BluefinTecsEcrSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
