...
{
  my $sdk = BluefinTecsEcrSDK->test({ 'net' => { 'offline' => 1 } }, undef);
...
{
  my $sdk = BluefinTecsEcrSDK->test({ 'net' => { 'failTimes' => 1, 'failStatus' => 503 } }, undef);
  my $res = $sdk->direct({ 'path' => '/ping' });
...
{
  my $sdk = BluefinTecsEcrSDK->test({ 'net' => { 'errorTimes' => 1 } }, undef);
  my $res = $sdk->direct({ 'path' => '/ping' });
...
{
  my $delay = 60;
  my $sdk = BluefinTecsEcrSDK->test({ 'net' => { 'latency' => $delay } }, undef);
...
{
  my $sdk = BluefinTecsEcrSDK->test(undef, undef);
