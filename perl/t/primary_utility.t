#!perl
# BluefinTecsEcr SDK primary utility test
#
...
#   live blessed context with BluefinTecsEcrOmni::livectx before calling a
...
my $_runner = BluefinTecsEcrOmni::make_runner($TEST_JSON, BluefinTecsEcrSDK->test(undef, undef));
...

{
  ...
  our @ISA = ('BluefinTecsEcrBaseFeature');

  sub new {
    my ($class, $fn) = @_;
    my $self = BluefinTecsEcrBaseFeature::new($class);
    $self->{hook_fn} = $fn;
    return $self;
  }
...
sub _err_from_map {
  ...
  return BluefinTecsEcrError->new($code, $msg);
}
...
sub _def_setup {
  ...
  return BluefinTecsEcrOmni::tostruct(Voxgig::Omni::Util::clone($setup));
}
...
runsection('done', sub {
  my ($view) = @_;
  return $utility->{done}->(BluefinTecsEcrOmni::livectx($view));
});
...
runsection('makeError', sub {
  my ($view, $errmap) = @_;
  my $ctx = BluefinTecsEcrOmni::livectx($view);
...

{
  ...
  $ctx->{result} = BluefinTecsEcrResult->new({
    'ok' => 0,
    'resdata' => { 'id' => 'safe01' },
  });
...

{
  ...
  my $feature = BluefinTecsEcrBaseFeature->new;
...
{
  my $hook_client = BluefinTecsEcrSDK->test(undef, undef);
...
{
  my $init_client = BluefinTecsEcrSDK->test(undef, undef);
...
{
  my $calls = [];
  my $live_client = BluefinTecsEcrSDK->new({
...
{
  my $blocked_client = BluefinTecsEcrSDK->new({
    'base' => 'http://localhost:8080',
    'system' => {
...
{
  my $ctx = make_test_full_ctx($client, $utility);
  $ctx->{spec} = BluefinTecsEcrSpec->new({
    'base' => 'http://localhost:8080',
    'prefix' => '/api',
    'path' => 'items/{id}',
    'suffix' => '',
...
runsection('makeResponse', sub {
  my ($view) = @_;
  return _unwrap($utility->{make_response}->(BluefinTecsEcrOmni::livectx($view)));
});
...
{
  my $ctx = make_test_full_ctx($client, $utility);
  $ctx->{spec} = BluefinTecsEcrSpec->new({ 'step' => 'start' });
  $ctx->{result} = undef;
  my (undef, $err) = $utility->{make_result}->($ctx);
...
{
  my $spec_client = BluefinTecsEcrSDK->test(undef, _def_setup('makeSpec'));
...
runsection('makeUrl', sub {
  my ($view) = @_;
  my $ctx = BluefinTecsEcrOmni::livectx($view);
  $ctx->{result} = BluefinTecsEcrResult->new({}) unless $ctx->{result};
...
runsection('operator', sub {
  my ($vin) = @_;
  $vin = {} unless ref($vin) eq 'HASH';
  my $op = BluefinTecsEcrOperation->new($vin);
  return {
    'entity' => $op->{entity},
    'name' => $op->{name},
    'input' => $op->{input},
...
runsection('param', sub {
  my ($view, $paramdef) = @_;
  return $utility->{param}->(BluefinTecsEcrOmni::livectx($view), $paramdef);
});
...
{
  my $auth_client = BluefinTecsEcrSDK->test(undef, _def_setup('prepareAuth'));
...
runsection('prepareBody', sub {
  my ($view) = @_;
  return $utility->{prepare_body}->(BluefinTecsEcrOmni::livectx($view));
});
...
runsection('prepareHeaders', sub {
  my ($view) = @_;
  return $utility->{prepare_headers}->(BluefinTecsEcrOmni::livectx($view));
});
...
runsection('prepareMethod', sub {
  my ($view) = @_;
  return $utility->{prepare_method}->(BluefinTecsEcrOmni::livectx($view));
});
...
runsection('prepareParams', sub {
  my ($view) = @_;
  return $utility->{prepare_params}->(BluefinTecsEcrOmni::livectx($view));
});
...
runsection('preparePath', sub {
  my ($view) = @_;
  return $utility->{prepare_path}->(BluefinTecsEcrOmni::livectx($view));
});
...
runsection('prepareQuery', sub {
  my ($view) = @_;
  return $utility->{prepare_query}->(BluefinTecsEcrOmni::livectx($view));
});
...
runsection('resultBasic', sub {
  my ($view) = @_;
  my $result = $utility->{result_basic}->(BluefinTecsEcrOmni::livectx($view));
...
runsection('resultBody', sub {
  my ($view) = @_;
  return $utility->{result_body}->(BluefinTecsEcrOmni::livectx($view));
});
...
runsection('resultHeaders', sub {
  my ($view) = @_;
  return $utility->{result_headers}->(BluefinTecsEcrOmni::livectx($view));
});
...
runsection('transformRequest', sub {
  my ($view) = @_;
  return $utility->{transform_request}->(BluefinTecsEcrOmni::livectx($view));
});
...
runsection('transformResponse', sub {
  my ($view) = @_;
  return $utility->{transform_response}->(BluefinTecsEcrOmni::livectx($view));
});
