...
package BluefinTecsEcrTestRunner;
...
sub is_control_skipped {
  ...
  my $skip = BluefinTecsEcrHelpers::gpath($ctrl, "test.skip.$mode") || {};
  my $items = $skip->{$kind} || [];
...
sub live_client_options {
  my $ctrl = load_test_control();
  my $opts = BluefinTecsEcrHelpers::gpath($ctrl, 'test.client.options');
...
sub live_delay_ms {
  my $ctrl = load_test_control();
  my $v = BluefinTecsEcrHelpers::gpath($ctrl, 'test.live.delayMs');
