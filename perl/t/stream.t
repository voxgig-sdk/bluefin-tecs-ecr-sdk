#!perl
# BluefinTecsEcr SDK entity stream() test
#
...
sub has_feature {
  my ($name) = @_;
  my $f = BluefinTecsEcrConfig::make_config()->{feature};
...
sub find_list_entity {
  ...
  my $entities = BluefinTecsEcrHelpers::to_map(
    BluefinTecsEcrHelpers::gp($config, 'entity')) || {};
  for my $name (sort keys %$entities) {
    my $points = BluefinTecsEcrHelpers::gpath($config, "entity.$name.op.list.points");
    next unless Voxgig::Struct::islist($points);
    for my $p (@$points) {
      my $params = BluefinTecsEcrHelpers::gpath($p, 'args.params');
      my $reqd = Voxgig::Struct::islist($params)
        ? scalar(grep { BluefinTecsEcrHelpers::is_true($_->{reqd}) } @$params)
        : 0;
...
SKIP: {
  ...
  {
    my $sdk = BluefinTecsEcrSDK->test({ 'entity' => $seed }, undef);
