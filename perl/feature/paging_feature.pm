# BluefinTecsEcr SDK paging feature
#
...
package BluefinTecsEcrPagingFeature;
...
sub init {
  ...
  $self->{active} = BluefinTecsEcrHelpers::is_true($self->{options}{active});
  return;
}
...
sub PreRequest {
  ...
  if ('graphql' eq (BluefinTecsEcrHelpers::gp($ctx->{point}, 'kind') // '')) {
...
sub _graphql_pre_request {
  ...
  my %declared;
  my $varlist = BluefinTecsEcrHelpers::gpath($ctx->{point}, 'graphql.vars');
  if (ref($varlist) eq 'ARRAY') {
    for my $v (@$varlist) {
      my $name = BluefinTecsEcrHelpers::gp($v, 'name');
...
sub PreResult {
  ...
  my $page = BluefinTecsEcrHelpers::gpath($ctx->{point}, 'graphql.page');
  if (ref($page) eq 'HASH' && Voxgig::Struct::ismap($body)) {
    ...
    if (defined $connpath && '' ne $connpath) {
      my $sub = BluefinTecsEcrHelpers::gpath($body, $connpath);
      $conn = $sub if defined $sub;
    }

    my $cursorpath = $page->{cursor};
    if (defined $cursorpath && '' ne $cursorpath) {
      my $cursor = BluefinTecsEcrHelpers::gpath($conn, $cursorpath);
      $paging->{cursor} = $cursor if defined $cursor;
    }

    my $morepath = $page->{more};
    if (defined $morepath && '' ne $morepath) {
      my $more = BluefinTecsEcrHelpers::gpath($conn, $morepath);
      if (BluefinTecsEcrHelpers::is_true($more) || BluefinTecsEcrHelpers::is_false($more)) {
        $paging->{hasMore} = BluefinTecsEcrHelpers::is_true($more) ? 1 : 0;
        $explicit_more = 1;
      }
    }
  }
  ...
  if (Voxgig::Struct::ismap($body)) {
    my $bnext = BluefinTecsEcrHelpers::gp($body, 'next');
    $paging->{next} = defined $paging->{next} ? $paging->{next} : $bnext
      if defined $bnext;
    my $bcursor = BluefinTecsEcrHelpers::gp($body, 'cursor');
    $paging->{cursor} = $bcursor if defined $bcursor;
    my $bnextcursor = BluefinTecsEcrHelpers::gp($body, 'nextCursor');
    $paging->{cursor} = $bnextcursor if defined $bnextcursor;
    my $bhasmore = $body->{hasMore};
    if (BluefinTecsEcrHelpers::is_true($bhasmore) || BluefinTecsEcrHelpers::is_false($bhasmore)) {
      $paging->{hasMore} = BluefinTecsEcrHelpers::is_true($bhasmore) ? 1 : 0;
      $explicit_more = 1;
    }
  }
