# BluefinTecsEcr SDK utility: make_spec
...
$REGISTRY{make_spec} = sub {
  ...
  my $base = BluefinTecsEcrHelpers::gp($options, 'base');
  $base = '' unless defined $base;
  my $prefix = BluefinTecsEcrHelpers::gp($options, 'prefix');
  $prefix = '' unless defined $prefix;
  my $suffix = BluefinTecsEcrHelpers::gp($options, 'suffix');
  $suffix = '' unless defined $suffix;

  my $parts = [];
  $parts = BluefinTecsEcrHelpers::gp($point, 'parts') if $point;
  ...
  my $allow_method = BluefinTecsEcrHelpers::gpath($options, 'allow.method');
  ...
  my $pkind = BluefinTecsEcrHelpers::gp($ctx->{point}, 'kind');
  if (defined $pkind && 'graphql' eq $pkind) {
    ...
    $ctx->{spec}{headers}{'content-type'} =
      $BluefinTecsEcrUtilities::GRAPHQL_CONTENT_TYPE;
  }
  else {
