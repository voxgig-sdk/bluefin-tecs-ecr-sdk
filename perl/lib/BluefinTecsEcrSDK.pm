...
sub new {
  ...
  my $utility = BluefinTecsEcrUtility->new;
  ...
  my $config = BluefinTecsEcrConfig::shared_config();
  ...
  if (BluefinTecsEcrHelpers::is_true(
    BluefinTecsEcrHelpers::gpath($self->{options}, 'feature.test.active'))) {
    $self->{mode} = 'test';
  }
  ...
  my $feature_opts = BluefinTecsEcrHelpers::to_map(
    BluefinTecsEcrHelpers::gp($self->{options}, 'feature')) || {};
  my $featureorder = BluefinTecsEcrHelpers::gpath(
    $self->{options}, '__derived__.featureorder');
  $featureorder = [] unless Voxgig::Struct::islist($featureorder);
  for my $fname (@$featureorder) {
    my $fopts = BluefinTecsEcrHelpers::to_map($feature_opts->{$fname});
    if ($fopts && BluefinTecsEcrHelpers::is_true($fopts->{active})) {
      $utility->{feature_add}->($self->{_rootctx},
        BluefinTecsEcrFeatures::make_feature($fname));
    }
  }

  # Add extension features.
  my $extend = BluefinTecsEcrHelpers::gp($self->{options}, 'extend');
...
sub get_utility {
  my ($self) = @_;
  return BluefinTecsEcrUtility->copy($self->{_utility});
}
...
sub prepare {
  ...
  my $ctrl = BluefinTecsEcrHelpers::to_map(
    BluefinTecsEcrHelpers::gp($fetchargs, 'ctrl')) || {};
  ...
  my $path = BluefinTecsEcrHelpers::gp($fetchargs, 'path');
  $path = '' unless defined $path && !ref $path;
  my $method_val = BluefinTecsEcrHelpers::gp($fetchargs, 'method');
  $method_val = 'GET' unless defined $method_val && !ref $method_val;
  my $params = BluefinTecsEcrHelpers::to_map(
    BluefinTecsEcrHelpers::gp($fetchargs, 'params')) || {};
  my $query = BluefinTecsEcrHelpers::to_map(
    BluefinTecsEcrHelpers::gp($fetchargs, 'query')) || {};
  my $headers = $utility->{prepare_headers}->($ctx);

  my $base = BluefinTecsEcrHelpers::gp($opts, 'base');
  $base = '' unless defined $base && !ref $base;
  my $prefix = BluefinTecsEcrHelpers::gp($opts, 'prefix');
  $prefix = '' unless defined $prefix && !ref $prefix;
  my $suffix = BluefinTecsEcrHelpers::gp($opts, 'suffix');
  ...
  $ctx->{spec} = BluefinTecsEcrSpec->new({
    ...
    'body' => BluefinTecsEcrHelpers::gp($fetchargs, 'body'),
    'step' => 'start',
  });

  # Merge user-provided headers.
  my $uh = BluefinTecsEcrHelpers::gp($fetchargs, 'headers');
...
sub _op_allowed {
  my ($self, $op) = @_;
  my $allow = BluefinTecsEcrHelpers::gpath($self->{options}, 'allow.op');
...
sub _op_denied {
  ...
  return {
    'ok' => 0,
    'err' => "BluefinTecsEcrSDK: $op: operation not allowed by" .
...
sub _raw_request {
  ...
  if (Voxgig::Struct::ismap($fetched)) {
    my $status = BluefinTecsEcrHelpers::to_int(
      BluefinTecsEcrHelpers::gp($fetched, 'status'));
    my $headers = BluefinTecsEcrHelpers::gp($fetched, 'headers') || {};
    ...
    my $json_data;
    unless ($no_body) {
      my $jf = BluefinTecsEcrHelpers::gp($fetched, 'json');
      if (ref $jf eq 'CODE') {
...
sub graphql {
  ...
  my $errors = BluefinTecsEcrHelpers::gpath($res, 'data.errors');
  ...
  if (ref $errors eq 'ARRAY' && 0 < scalar @$errors) {
    ...
    $res->{'ok'} = 0;
    $res->{'err'} = "BluefinTecsEcrSDK: graphql: $msg";
    $res->{'graphql'} = $errors;
  }
...
# Canonical facade: $client->EcrApi->list / ->load({ 'id' => ... })
sub EcrApi {
  my ($self, $data) = @_;
  require(Cwd::abs_path("$DIR/../entity/ecr_api_entity.pm"));
  return EcrApiEntity->new($self, $data);
}
