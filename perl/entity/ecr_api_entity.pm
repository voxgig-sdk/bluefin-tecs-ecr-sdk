# BluefinTecsEcr SDK EcrApi entity
...
sub new {
  ...
  elsif (BluefinTecsEcrHelpers::is_false($entopts->{active})) {
    # keep false
  }
  else {
    $entopts->{active} = Voxgig::Struct::JTRUE();
...
sub make {
  ...
  return EcrApiEntity->new($self->{_client}, $opts);
}

sub data_set {
  my ($self, $args) = @_;
  if ($args) {
    $self->{_data} = BluefinTecsEcrHelpers::to_map(Voxgig::Struct::clone($args)) || {};
    $self->{_utility}{feature_hook}->($self->{_entctx}, 'SetData');
  }
  return;
}

# Returns the current EcrApi data (hashref).
sub data_get {
  my ($self) = @_;
...
sub match_set {
  my ($self, $args) = @_;
  if ($args) {
    $self->{_match} = BluefinTecsEcrHelpers::to_map(Voxgig::Struct::clone($args)) || {};
    $self->{_utility}{feature_hook}->($self->{_entctx}, 'SetMatch');
  }
  return;
}

# Returns the current match filter (any subset of EcrApi fields).
sub match_get {
  my ($self) = @_;
...
# Load a single EcrApi.
#
# reqmatch: match criteria hashref (id/query fields; EcrApiLoadMatch
...
# Returns the loaded EcrApi data (hashref); dies with BluefinTecsEcrError
# on failure.
sub load {
  ...
  return $self->_run_op($ctx, sub {
    ...
    if ($result) {
      ...
      if ($result->{resdata}) {
        $self->{_data} = BluefinTecsEcrHelpers::to_map(
...
# Create a new EcrApi.
#
# reqdata: body data hashref (EcrApiCreateData shape). ctrl: optional
# per-call control. Returns the created EcrApi data (hashref); dies
# with BluefinTecsEcrError on failure.
sub create {
...
sub stream {
  ...
  my $utility = $self->{_utility};
  $callopts = BluefinTecsEcrHelpers::to_map($callopts) || {};
  my $signal = BluefinTecsEcrHelpers::gp($callopts, 'signal');
  my $ctrl = BluefinTecsEcrHelpers::to_map(
    BluefinTecsEcrHelpers::gp($callopts, 'ctrl')) || {};
  ...
  my $ctx = $utility->{make_context}->({
    ...
    'match' => $self->{_match},
    'data' => $self->{_data},
    %{ BluefinTecsEcrHelpers::to_map($args) || {} },
  }, $self->{_entctx});

  # Outbound: expose an async-iterable/list payload so the request builder /
  # transport can stream it as the request body.
  my $body = BluefinTecsEcrHelpers::gp($callopts, 'body');
  if (defined $body) {
    my $reqdata = BluefinTecsEcrHelpers::to_map($ctx->{reqdata}) || {};
    $reqdata->{'body$'} = $body;
    $ctx->{reqdata} = $reqdata;
    $ctx->{stream_out} = $body;
  }
