// BluefinTecsEcr SDK — direct unit tests for the operation-pipeline utilities
...
static std::shared_ptr<BluefinTecsEcrSDK> plClient(const Value& sdkopts) {
  return BluefinTecsEcrSDK::testSDK(Value::undef(), sdkopts);
}

static CtxPtr plCtx(std::shared_ptr<BluefinTecsEcrSDK> client, UtilityPtr utility, const Value& ctrl) {
  CtxSpec cs;
  cs.setOpname("load");
...
static std::string names(std::shared_ptr<BluefinTecsEcrSDK> client) {
  std::string out;
