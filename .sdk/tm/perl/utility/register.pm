...
$BluefinTecsEcrUtility::REGISTRAR = sub {
  my ($u) = @_;
  for my $k (keys %BluefinTecsEcrUtilities::REGISTRY) {
    $u->{$k} = $BluefinTecsEcrUtilities::REGISTRY{$k};
  }
  # The vendored struct utility, reachable as $utility->{struct}{<fn>}.
  $u->{struct} = BluefinTecsEcrHelpers::struct_facade();
};
