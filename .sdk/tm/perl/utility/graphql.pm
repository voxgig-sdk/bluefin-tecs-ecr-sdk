# BluefinTecsEcr SDK utility: graphql
#
...
sub graphql_error_code {
  ...
  my $ext = BluefinTecsEcrHelpers::gp($gqlerr, 'extensions');

  my $raw = BluefinTecsEcrHelpers::gp($ext, 'code');
  $raw = BluefinTecsEcrHelpers::gp($ext, 'type') if !defined $raw || '' eq $raw;
...
$REGISTRY{graphql_body} = sub {
  ...
  my $gql = BluefinTecsEcrHelpers::gp($ctx->{point}, 'graphql');
  ...
  my $varlist = BluefinTecsEcrHelpers::gp($gql, 'vars');
  $varlist = [] unless ref($varlist) eq 'ARRAY';

  for my $spec (@$varlist) {
    next unless ref($spec) eq 'HASH';

    my $name = BluefinTecsEcrHelpers::gp($spec, 'name');
    my $from = BluefinTecsEcrHelpers::gp($spec, 'from');
    ...
    my $val = BluefinTecsEcrHelpers::gp($reqsrc, $from);
    $val = BluefinTecsEcrHelpers::gp($datasrc, $from) unless defined $val;
    $variables{$name} = $val if defined $val;
  }

  return {
    query     => BluefinTecsEcrHelpers::gp($gql, 'doc'),
    variables => \%variables,
  };
};
...
$REGISTRY{graphql_errors} = sub {
  ...
  my $kind = BluefinTecsEcrHelpers::gp($ctx->{point}, 'kind');
  return 0 unless defined $kind && 'graphql' eq $kind;

  my $errors = BluefinTecsEcrHelpers::gp($result->{body}, 'errors');
  return 0 unless ref($errors) eq 'ARRAY' && 0 < scalar(@$errors);

  my $first = $errors->[0];
  my $msg = BluefinTecsEcrHelpers::gp($first, 'message');
