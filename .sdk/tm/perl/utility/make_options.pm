# BluefinTecsEcr SDK utility: make_options
...
$REGISTRY{make_options} = sub {
  ...
  my $custom_utils = BluefinTecsEcrHelpers::gp($options, 'utility');
  ...
  my $extend_raw = BluefinTecsEcrHelpers::gp($options, 'extend');
  ...
  BluefinTecsEcrHelpers::coerce_bools($opts);
  ...
  my $sys_fetch = BluefinTecsEcrHelpers::gpath($opts, 'system.fetch');

  # CLONE the config side: `config` is a process-wide singleton
  # (BluefinTecsEcrConfig::shared_config) and merge uses its nested hashes as merge
  ...
  my $clean_keys = BluefinTecsEcrHelpers::gpath($opts, 'clean.keys');
  ...
  if (!@featureorder) {
    my $fmap = BluefinTecsEcrHelpers::to_map(
      BluefinTecsEcrHelpers::gp($opts, 'feature')) || {};
