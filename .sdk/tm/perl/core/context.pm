...
sub new {
  ...
  my $gcp = \&BluefinTecsEcrHelpers::get_ctx_prop;
  ...
  $self->{ctrl} = BluefinTecsEcrControl->new();
  ...
  $self->{data} = BluefinTecsEcrHelpers::to_map($gcp->($ctxmap, 'data')) || {};
  $self->{reqdata} = BluefinTecsEcrHelpers::to_map($gcp->($ctxmap, 'reqdata')) || {};
  $self->{match} = BluefinTecsEcrHelpers::to_map($gcp->($ctxmap, 'match')) || {};
  $self->{reqmatch} = BluefinTecsEcrHelpers::to_map($gcp->($ctxmap, 'reqmatch')) || {};

  my $pt = $gcp->($ctxmap, 'point');
  $self->{point} = Voxgig::Struct::ismap($pt) ? $pt : ($basectx ? $basectx->{point} : undef);

  my $sp = $gcp->($ctxmap, 'spec');
  $self->{spec} = (Scalar::Util::blessed($sp) && $sp->isa('BluefinTecsEcrSpec'))
    ? $sp : ($basectx ? $basectx->{spec} : undef);

  my $r = $gcp->($ctxmap, 'result');
  $self->{result} = (Scalar::Util::blessed($r) && $r->isa('BluefinTecsEcrResult'))
    ? $r : ($basectx ? $basectx->{result} : undef);

  my $rp = $gcp->($ctxmap, 'response');
  $self->{response} = (Scalar::Util::blessed($rp) && $rp->isa('BluefinTecsEcrResponse'))
...
sub resolve_op {
  ...
  return BluefinTecsEcrOperation->new({}) if '' eq $opname;

  my $opcfg = BluefinTecsEcrHelpers::gpath($self->{config}, "entity.$entname.op.$opname");

  my $input = ($opname eq 'update' || $opname eq 'create') ? 'data' : 'match';

  my $points = [];
  if (Voxgig::Struct::ismap($opcfg)) {
    my $t = BluefinTecsEcrHelpers::gp($opcfg, 'points');
    $points = $t if Voxgig::Struct::islist($t);
  }

  my $op = BluefinTecsEcrOperation->new({
    entity => $entname,
    name   => $opname,
    input  => $input,
    points => $points,
...
sub make_error {
  my ($self, $code, $msg) = @_;
  return BluefinTecsEcrError->new($code, $msg, $self);
}
