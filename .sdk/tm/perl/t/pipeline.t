...
sub resp {
  ...
  return BluefinTecsEcrResponse->new({
    'status' => $status,
...
sub is_sdk_error {
  my ($err) = @_;
  return Scalar::Util::blessed($err) && $err->isa('BluefinTecsEcrError');
}
...

{
  ...
  $ctx->{spec} = spec_of();
  $ctx->{result} = BluefinTecsEcrResult->new({ 'ok' => 1, 'resdata' => [] });
  my ($result, $err) = $utility->{make_result}->($ctx);
...
{
  my $ctx = make_ctx();
  my $preset = BluefinTecsEcrResult->new({ 'ok' => 1 });
...
{
  my $boom = BluefinTecsEcrError->new('boom', 'boom');
...

{
  ...
  $u->{make_fetch_def} = sub {
    return (undef, BluefinTecsEcrError->new('fetchdef_boom', 'boom'));
  };
  my $ctx = make_ctx(utility => $u);
  $ctx->{spec} = request_spec();
  my ($response, $err) = $u->{make_request}->($ctx);
...
{
  my $ctx = make_ctx();
  $ctx->{result} = BluefinTecsEcrResult->new({ 'ok' => 1, 'resdata' => 42 });
  is($utility->{done}->($ctx), 42, 'done returns resdata on success');
}

{
  my $ctx = make_ctx();
  $ctx->{result} = BluefinTecsEcrResult->new({ 'ok' => 0 });
...

{
  ...
  $ctx->{result} = BluefinTecsEcrResult->new({ 'ok' => 1, 'resdata' => 7 });
...

{
  ...
  $ctx->{result} = BluefinTecsEcrResult->new({ 'ok' => 0, 'resdata' => 'fallback' });
...
{
  my $oclient = BluefinTecsEcrSDK->test(undef, undef);
  ...
  $oclient->{features} = [];
  my $a = BluefinTecsEcrBaseFeature->new;
  my $b = BluefinTecsEcrBaseFeature->new;
...
sub named_feature {
  my ($name) = @_;
  my $f = BluefinTecsEcrBaseFeature->new;
  $f->{name} = $name;
  return $f;
}
...

{
  ...
  ok(BluefinTecsEcrHelpers::is_true($o2->{feature}{metrics}{active}),
    'feature order: array normalized to a map (metrics opts preserved)');
  ok(BluefinTecsEcrHelpers::is_true($o2->{feature}{test}{active}),
...
sub auth_ctx {
  ...
  my $ctx = BluefinTecsEcrContext->new({
    'client' => PipelineOptClient->new($options),
    'utility' => $utility,
    'opname' => 'load',
  }, undef);
  $ctx->{spec} = defined $headers
    ? BluefinTecsEcrSpec->new({ 'headers' => $headers, 'step' => 's' })
    : undef;
  return $ctx;
}
...
{
  my $ctx = make_ctx();
  $ctx->{response} = BluefinTecsEcrResponse->new({ 'status' => 200 });
  $ctx->{result} = BluefinTecsEcrResult->new({});
...
{
  my $ctx = make_ctx();
  $ctx->{response} = BluefinTecsEcrResponse->new({
