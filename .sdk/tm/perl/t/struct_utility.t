...
my $runner = BluefinTecsEcrOmni::make_runner($corpus_path, $client);
my $R = $runner->('struct');
...
my $NULLMARK = BluefinTecsEcrOmni::NULLMARK();
...
{
  my $td = BluefinTecsEcrOmni::tostruct(
...
{
  my $basic = BluefinTecsEcrOmni::tostruct(
...
grun('inject.string', $inject_spec->{string},
  sub { $S->{inject}->($_[0]{val}, $_[0]{store},
    { modify => \&BluefinTecsEcrOmni::null_modifier }) });
