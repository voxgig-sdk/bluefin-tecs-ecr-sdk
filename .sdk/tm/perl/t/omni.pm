...
#    `contextify` answers a TIED plain hash (BluefinTecsEcrOmniViewTie) whose
...
package BluefinTecsEcrOmniEntityRef;
...
# --------------------------------------------------------------------------
package BluefinTecsEcrOmniViewTie;
...
sub _iserr { return Scalar::Util::blessed($_[0]) && $_[0]->isa('BluefinTecsEcrError') }
...
sub _wrap {
  ...
  if ('ok' eq $name && Scalar::Util::blessed($t) && $t->isa('BluefinTecsEcrResult')
    && defined $val && !ref $val) {
  ...
  if (ref($val) eq 'HASH' && tied(%$val)
    && tied(%$val)->isa('BluefinTecsEcrOmniViewTie')) {
    my $inner = tied(%$val)->{target};
    return $inner if $seen->{ Scalar::Util::refaddr($inner) };
    return BluefinTecsEcrOmni::objview($inner,
  ...
  if (Scalar::Util::blessed($val)) {
    ...
    if (('HASH' eq (Scalar::Util::reftype($val) || ''))) {
      ...
      return BluefinTecsEcrOmni::objview($val,
        { %$seen, Scalar::Util::refaddr($t) => 1 });
    }
  ...
  if (ref($val) eq 'HASH' || ref($val) eq 'ARRAY') {
    return BluefinTecsEcrOmni::toomni($val);
  }
...
# --------------------------------------------------------------------------
package BluefinTecsEcrOmniProviderTie;
...
package BluefinTecsEcrOmniProvider;
...
sub objview {
  my ($target, $seen) = @_;
  tie my %view, 'BluefinTecsEcrOmniViewTie', $target, $seen;
  return \%view;
}
...
sub isview {
  ...
  return ref($val) eq 'HASH' && tied(%$val)
    && tied(%$val)->isa('BluefinTecsEcrOmniViewTie') ? 1 : 0;
}
...
sub sdkhooks {
  ...
  my $struct = $utility->{struct} || BluefinTecsEcrHelpers::struct_facade();
  ...
  my $subject = sub {
    ...
    $found = $utility->{ BluefinTecsEcrOmniViewTie::_snake($name) } if !defined $found;
  ...
  my $contextify = sub {
    ...
    if (Voxgig::Struct::ismap($ent) && !Scalar::Util::blessed($ent)
      ...
      $ctxmap->{entity} = BluefinTecsEcrOmniEntityRef->new($ent->{name});
    }
    my $ctx = BluefinTecsEcrContext->new($ctxmap, undef);
    enrich($ctxmap, $ctx);
    $ctx->{utility} = $utility;
...
sub sdkprovider {
  my ($sdk) = @_;
  tie my %provider, 'BluefinTecsEcrOmniProviderTie', $sdk, sdkhooks($sdk);
  return bless \%provider, 'BluefinTecsEcrOmniProvider';
}
