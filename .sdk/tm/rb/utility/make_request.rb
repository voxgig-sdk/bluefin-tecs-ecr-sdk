...
module BluefinTecsEcrUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = BluefinTecsEcrResponse.new({})
    ...
    if fetch_err
      ...
      response = BluefinTecsEcrResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = BluefinTecsEcrResponse.new(fetched)
    else
