...
defmodule BluefinTecsEcr.PrimaryUtilityTest do
  ...
  alias BluefinTecsEcr.{Context, Utility, Spec, Result}

  defp client, do: BluefinTecsEcr.test()

  defp ctx(client, opname \\ "load") do
    Context.new(S.jm(["opname", opname]), BluefinTecsEcr.get_root_ctx(client))
  end
  ...
  test "prepare_auth sets an authorization header when an apikey is present" do
    c = BluefinTecsEcr.new(H.deep(%{"apikey" => "secret"}))
    ctx = ctx(c)
  ...
  test "make_options derives a clean key regex" do
    c = BluefinTecsEcr.test()
    opts = BluefinTecsEcr.options_map(c)
