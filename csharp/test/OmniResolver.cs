...
// `Util` alone would resolve to the SDK's BluefinTecsEcrSdk.Util namespace,
...
public static class OmniResolver
{
        ...
        var provider = new Provider
        {
            ...
            Client = options =>
            {
                var opts = FixNums(options) as Dictionary<string, object?>
                    ?? new Dictionary<string, object?>();
                var sub = SdkProvider(BluefinTecsEcrSDK.TestSDK(null, opts));
                MarkDefProvider(sub);
                return sub;
            },
            ...
            Errify = err =>
            {
                if (err is BluefinTecsEcrError sdkerr)
                {
                    var outmap = new Dictionary<string, object?>
                    {
    ...
    /// </summary>
    public static Context OmniCtx(object? arg, BluefinTecsEcrSDK client, Utility utility)
    {
        ...
        if (ctxmap.TryGetValue("client", out var p) && p is Provider prov &&
            DefProviderClient(prov) is BluefinTecsEcrSDK live)
        {
            client = live;
