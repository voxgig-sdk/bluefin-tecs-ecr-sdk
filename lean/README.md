# BluefinTecsEcr Lean SDK



The Lean SDK for the BluefinTecsEcr API — an entity-oriented client
...
[Releases](https://github.com/voxgig-sdk/bluefin-tecs-ecr-sdk/releases)) or from a source checkout. The runtime is
...
def main : IO Unit := do
  let ctx ← mkCtx
  (do
    let opts ← newMap #[("base", .str "https://api.example.com"), ("apikey", .str (← IO.getEnv "BLUEFIN_TECS_ECR").getD "")]
    let sdk ← Sdk.newSdk opts
    let result ← Ecr_api.create sdk (← emptyMap) (← emptyMap)
