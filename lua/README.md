# BluefinTecsEcr Lua SDK



The Lua SDK for the BluefinTecsEcr API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:EcrApi()` — each with the same small set of operations (`load`, `create`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
GitHub release tag (`lua/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/bluefin-tecs-ecr-sdk/releases)),
...
```lua
-- Create
local created, err = client:EcrApi():create({ amount = "example_amount", card_number = "example_card_number", currency = "example_currency", terminal_number = "example_terminal_number", transaction_date_time = "example_transaction_date_time", transaction_id = "example_transaction_id" })
if err then error(err) end
...
Check `err` first (it is non-`nil` on failure), then use `value`:
    ...
    local ecr_api, err = client:EcrApi():load()
    if err then error(err) end
    -- ecr_api is the loaded record
...
Create an instance: `local ecr_api = client:EcrApi(nil)`
...
| `payment_reasonAsByte` | `table` | Payment reason represented as a byte array. |
...
```lua
local ecr_api, err = client:EcrApi():create({
...
```
lua/
├── bluefin-tecs-ecr_sdk.lua    -- Main SDK module
...
The main module (`bluefin-tecs-ecr_sdk`) exports the SDK constructor
