# bluefin-tecs-ecr-mcp

[MCP](https://modelcontextprotocol.io) server exposing the BluefinTecsEcr SDK as
two agent tools — `bluefin-tecs-ecr_list` and `bluefin-tecs-ecr_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/bluefin-tecs-ecr-mcp)
make build
...
claude mcp add --scope user bluefin-tecs-ecr \
  -- /absolute/path/to/bluefin-tecs-ecr-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./bluefin-tecs-ecr-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// bluefin-tecs-ecr_list: first page of records
{ "entity": "ecr_api" }
{ "entity": "ecr_api", "query": { } }

// bluefin-tecs-ecr_load: one record by id
{ "entity": "ecr_api", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/bluefin-tecs-ecr-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user bluefin-tecs-ecr \
     -- "$PWD"/dist/*/bluefin-tecs-ecr-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `bluefin-tecs-ecr_list` and `bluefin-tecs-ecr_load` tools now appear
   in new sessions. Ask the agent to *"list ecr_api using bluefin-tecs-ecr"*
   and it calls `bluefin-tecs-ecr_list` with `{"entity":"ecr_api"}`.
...
### Call the `bluefin-tecs-ecr_list` tool
...
### Call the `bluefin-tecs-ecr_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `bluefin-tecs-ecr_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `bluefin-tecs-ecr_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
```sh
./bluefin-tecs-ecr-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"bluefin-tecs-ecr_load","arguments":{"entity":"ecr_api","query":{"id":1}}}}'
```
