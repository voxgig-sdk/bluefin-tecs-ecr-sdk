// EcrApi entity client (generated — mirrors the go/rust Entity fragment).
...
pub const EntResult = union(enum) {
    ok: *EcrApiEntity,
    err: *errmod.BluefinTecsEcrError,
};
...
pub const EcrApiEntity = struct {
    name: []const u8 = "ecr_api",
    client: *sdk.BluefinTecsEcrSDK,
    utility: *Utility,
    entopts: Value,
    data: Value,
    mtch: Value,
    ...
    pub fn new(client: *sdk.BluefinTecsEcrSDK, entopts_in: Value) *EcrApiEntity {
        ...
        const e = h.A().create(EcrApiEntity) catch unreachable;
        e.* = .{
            .name = "ecr_api",
            .client = client,
    ...
    fn ent_ctx(self: *EcrApiEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *EcrApiEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *EcrApiEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *EcrApiEntity, ctx: *Context, post_done: *const fn (*EcrApiEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *EcrApiEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *EcrApiEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *EcrApiEntity, ctx: *Context, post_done: *const fn (*EcrApiEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *EcrApiEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *EcrApiEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return EcrApiEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *EcrApiEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *EcrApiEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *EcrApiEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *EcrApiEntity, reqmatch: Value, ctrl: Value) EntResult {
    ...
    fn load_post_done(self: *EcrApiEntity, ctx: *Context) void {
    ...
    pub fn list(self: *EcrApiEntity, _reqmatch: Value, _ctrl: Value) EntListResult {
        _ = _reqmatch;
        _ = _ctrl;
    ...
    pub fn create(self: *EcrApiEntity, reqdata: Value, ctrl: Value) EntResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "create",
            .ctrl = ctrl,
            .mtch = self.mtch,
    ...
    fn create_post_done(self: *EcrApiEntity, ctx: *Context) void {
        if (ctx.result) |result| {
            const resdata = result.resdata;
            if (!h.is_noval(resdata)) {
                const cm = h.to_map(h.clone(resdata));
    ...
    pub fn update(self: *EcrApiEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *EcrApiEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
