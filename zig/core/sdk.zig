// BluefinTecsEcrSDK client (generated — mirrors the go/rust Main fragment).
...
pub const BluefinTecsEcrSDK = struct {
    mode: []const u8 = "live",
    options: Value = .{ .null = {} },
    ...
    pub fn new(options: Value) *BluefinTecsEcrSDK {
    ...
    pub fn new_with(options: Value, extend: []const Feature) *BluefinTecsEcrSDK {
        const sdk = h.A().create(BluefinTecsEcrSDK) catch unreachable;
        sdk.* = .{
            .mode = "live",
            .options = h.vnull(),
    ...
    pub fn options_map(self: *BluefinTecsEcrSDK) Value {
        return h.clone(self.options);
    }

    pub fn get_utility(self: *BluefinTecsEcrSDK) *Utility {
        return Utility.copy(self.sdkUtility);
    }

    pub fn get_root_ctx(self: *BluefinTecsEcrSDK) *Context {
        return self.rootctx orelse unreachable;
    }

    pub fn prepare(self: *BluefinTecsEcrSDK, fetchargs_in: Value) E!Value {
    ...
    pub fn direct(self: *BluefinTecsEcrSDK, fetchargs_in: Value) Value {
    ...
    fn op_allowed(self: *BluefinTecsEcrSDK, op: []const u8) bool {
    ...
    fn op_denied(self: *BluefinTecsEcrSDK, op: []const u8) Value {
        const allow: []const u8 = switch (h.getpath(&.{ "allow", "op" }, self.options)) {
            .string => |s| s,
            else => "",
        };
        const msg = std.fmt.allocPrint(h.A(),
            "BluefinTecsEcrSDK: {s}: operation not allowed by" ++
    ...
    // gate by passing it.
    fn raw_request(self: *BluefinTecsEcrSDK, fetchargs_in: Value) Value {
    ...
    pub fn graphql(
        self: *BluefinTecsEcrSDK, query: []const u8, variables: Value, ctrl: Value,
    ) Value {
        ...
        if (errors == .array and 0 < errors.array.data.items.len) {
            ...
            const msg = std.fmt.allocPrint(h.A(),
                "BluefinTecsEcrSDK: graphql: {s}", .{m}) catch "";
    ...
    pub fn ecr_api(self: *@This(), entopts: Value) *@import("../entity/ecr_api.zig").EcrApiEntity {
        return @import("../entity/ecr_api.zig").EcrApiEntity.new(self, entopts);
    }

};

pub fn test_sdk(testopts_in: Value, sdkopts_in: Value) *BluefinTecsEcrSDK {
    ...
    const sdk = BluefinTecsEcrSDK.new(sdkopts);
    sdk.mode = "test";
