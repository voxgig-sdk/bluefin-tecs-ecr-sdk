# BluefinTecsEcr SDK test runner
...
class BluefinTecsEcrTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            BluefinTecsEcrTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = BluefinTecsEcrTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = BluefinTecsEcrTestRunner.getenv("BLUEFIN_TECS_ECR_TEST_LIVE")
        override = BluefinTecsEcrTestRunner.getenv("BLUEFIN_TECS_ECR_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = BluefinTecsEcrTestRunner.getenv(key)
        ...
        explain = BluefinTecsEcrTestRunner.getenv("BLUEFIN_TECS_ECR_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if BluefinTecsEcrTestRunner._test_control is not None:
            return BluefinTecsEcrTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                BluefinTecsEcrTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            BluefinTecsEcrTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = BluefinTecsEcrTestRunner.load_test_control()
...
def env_override(m):
    return BluefinTecsEcrTestRunner.env_override(m)


def entity_data(v):
    return BluefinTecsEcrTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return BluefinTecsEcrTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return BluefinTecsEcrTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return BluefinTecsEcrTestRunner.load_test_control()


def live_client_options():
    return BluefinTecsEcrTestRunner.live_client_options()


def live_delay_ms():
    return BluefinTecsEcrTestRunner.live_delay_ms()
