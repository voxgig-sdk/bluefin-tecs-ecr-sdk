...
class TestEcrApiDirect:

    def test_should_direct_load_ecr_api(self):
        setup = _ecr_api_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-ecr_api", "live" if setup["live"] else "unit")
        if _skip:
...
def _ecr_api_direct_setup(mockres):
